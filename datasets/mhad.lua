...
local M = {}
local MHADDataset = torch.class('resnet.MHADDataset', M)

function MHADDataset:__init(imageInfo, opt, split)
...
function MHADDataset:size()
   return self.imageInfo.data:size(1)
end

-- Computed from entire MHAD training set
function MHADDataset:preprocess()
